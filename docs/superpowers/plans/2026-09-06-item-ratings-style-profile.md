...
- [ ] **Step 1: Write failing schema tests** for all accepted half-star values and rejection of `0`, `5.25`, `-1`, and `3.2`.
- [ ] **Step 2: Run** `set -a; source .env.release; set +a; docker compose --env-file .env.release -f docker-compose.release.yml run --rm --no-deps -e TEST_DATABASE_URL="postgresql+asyncpg://wardrobe:${POSTGRES_PASSWORD}@postgres:5432/wardrobe_test" -v "$PWD/backend:/app" backend python -m pytest tests/test_item_ratings.py -q`; verify validation/model failures.
- [ ] **Step 3: Add the additive Alembic migration, SQLAlchemy columns, and a reusable Pydantic half-star validator wired into item create/update/response schemas.**
- [ ] **Step 4: Run the focused tests and migration-head check; verify they pass.**
- [ ] **Step 5: Commit** `feat: add item fit and style scores`.
...
- [ ] **Step 1: Add a failing authenticated API test in `backend/tests/test_items.py`** that patches both scores, reloads the item, and asserts the response and database values are `4.5` and `3.0`.
- [ ] **Step 2: Run the focused API test and verify it fails because the model/schema fields are not yet wired through the update path.**
- [ ] **Step 3: Wire the fields through service update/create handling and response serialization without changing unrelated item fields.**
- [ ] **Step 4: Run the focused API test plus existing item API tests; verify they pass.**
- [ ] **Step 5: Commit** `feat: persist item rating scores through api`.
...
- [ ] **Step 1: Add failing learning tests** with rated and unrated items; assert unrated items are ignored, scores normalize with `(score - 3) / 2`, and colour/type/brand/style aggregates include the rated item signal.
- [ ] **Step 2: Run the focused tests and verify failures for missing fields/aggregation.**
- [ ] **Step 3: Add the migration/model counters and extend recomputation with direct-item accumulators, a minimum supporting count of three for item-specific claims, and confidence based on sample count.**
- [ ] **Step 4: Extend learning response schemas and frontend types, then run focused learning/API tests.**
- [ ] **Step 5: Commit** `feat: learn preferences from item ratings`.
...
- [ ] **Step 1: Write failing component tests** for empty state, half/full rendering, click selection, ArrowLeft/ArrowRight keyboard changes, and accessible label/value.
- [ ] **Step 2: Run** the focused Vitest file and verify it fails because the picker does not exist.
- [ ] **Step 3: Implement the picker with Lucide stars, half-fill styling, keyboard semantics, and no network side effects.**
- [ ] **Step 4: Add Fit and Style controls to the item detail dialog using `useUpdateItem`; add compact read-only indicators to item cards; update `Item` types.**
- [ ] **Step 5: Run the focused frontend tests and relevant wardrobe/component tests; verify they pass.**
- [ ] **Step 6: Commit** `feat: add item fit and style rating controls`.
...
- [ ] **Step 1: Add failing backend/frontend tests** for the refresh action and a style insight supported by at least three rated items.
- [ ] **Step 2: Run focused tests and verify failures.**
- [ ] **Step 3: Implement the metrics display, refresh action, and explainable insight formatting; use configured local AI only to rewrite supplied facts, with deterministic fallback text when unavailable.**
- [ ] **Step 4: Run focused tests and existing learning-page tests; verify they pass.**
- [ ] **Step 5: Commit** `feat: surface explainable style profile`.
...
- [ ] **Step 1: Run the backend focused suites, frontend focused suites, migration-head check, and release compose config check.**
- [ ] **Step 2: Rebuild and restart with `./wardrobe release up`; verify `./wardrobe release doctor`.**
- [ ] **Step 3: Use the UI to set one fit and one style half-star rating, recompute the profile, and verify the values survive a release restart.**
- [ ] **Step 4: Run `./wardrobe release backup /tmp/wardrowbe-release-2026-09-06-ratings` and verify the portable backup contains the updated database dump.**
- [ ] **Step 5: Commit** `docs: document item ratings and style profile` and push the branch.
